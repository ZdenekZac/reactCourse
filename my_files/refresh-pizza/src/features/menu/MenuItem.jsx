...
function MenuItem({ pizza }) {
  ...
  return (
    <li key={id} alt={name} className='h-24 flex gap-4 py-2'>
      <img src={imageUrl} alt={name} className={`h-24 ${pizza.soldOut ? 'opacity-70 grayscale' : ''} flex`} />
      <div className='flex grow flex-col pt-0.5'>
        <p className='font-medium'>{name}</p>
        <p className='text-sm captalize italic text-stone-500'> {ingredients.join(', ')}</p>
